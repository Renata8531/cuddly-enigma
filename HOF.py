...
FILES = {
    ...
    "competences": DATA_DIR / "competences_referentiel.csv",
...
st.title("HOF - Gestion formation V0.5")
...
# ============================================================
    ...
    if programmes.empty:
        ...
        if comps_ref.empty:
            ...
            for _, comp in visible_comps.iterrows():
                comp_id = comp["competence_id"]

                already = not existing[
                    (existing["competence_id"] == comp_id) &
                    (existing["prevue"].astype(str) == "True")
                ].empty

                label = f"{comp['epreuve']} | {comp['code_competence']} - {comp['competence']}"
                checked = st.checkbox(label, value=already, key=f"prog_{programme_id}_{comp_id}")
...
# ============================================================
    ...
    st.header("Documents à générer / archiver")

    st.info("Cette V0 prépare les données. La génération PDF automatique viendra ensuite.")
    ...
    documents = [
        ...
        "Évaluation satisfaction stagiaire",
        "Auto-évaluation stagiaire",
        "Bilan formateur",
        "Dossier Qualiopi session",
    ]

    df_docs = pd.DataFrame({
        "document": documents,
        "statut": ["À générer"] * len(documents)
    })

    st.dataframe(df_docs, use_container_width=True)

    st.download_button(
        "Télécharger la checklist documents CSV",
        df_docs.to_csv(index=False, encoding="utf-8-sig"),
        "checklist_documents.csv",
        "text/csv"
    )
