...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### 🚀 Outils")
    st.page_link("pages/1_Calendrier.py",   label="📅 Calendrier formations")
    st.page_link("pages/2_Documents.py",    label="📄 Génération PDF")
    st.page_link("pages/3_Emargement.py",   label="✍️ QR Émargement")
    st.page_link("pages/4_Envoi_liens.py",  label="📧 Envoi liens stagiaires")
    st.page_link("pages/5_Signature.py",    label="🔏 Signature électronique")
...
if menu == "Tableau de bord":
    ...
    c1.page_link("pages/1_Calendrier.py",  label="📅 Voir le calendrier",      use_container_width=True)
    c2.page_link("pages/2_Documents.py",   label="📄 Générer des PDF",          use_container_width=True)
    c3.page_link("pages/3_Emargement.py",  label="✍️ QR codes émargement",      use_container_width=True)
    c4, c5, _ = st.columns(3)
    c4.page_link("pages/4_Envoi_liens.py", label="📧 Envoyer aux stagiaires",   use_container_width=True)
    c5.page_link("pages/5_Signature.py",   label="🔏 Signature électronique",   use_container_width=True)
